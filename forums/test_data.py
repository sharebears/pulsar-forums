...
class ForumsPopulator(TestDataPopulator):
    ...
    def populate(cls):
        ...
        db.session.execute("ALTER SEQUENCE forums_threads_notes_id_seq RESTART WITH 4")
        db.session.execute(
        ...
        db.session.execute(
            ...
            (5, 2, 'Choice B'),
            (6, 3, 'Choice A')""")
        db.session.execute("ALTER SEQUENCE forums_polls_choices_id_seq RESTART WITH 7")
        db.session.execute(
