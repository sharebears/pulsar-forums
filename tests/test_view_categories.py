...
def test_view_categories(app, authed_client):
    ...
    check_json_response(
        response,
        {'id': 1, 'name': 'Site', 'description': 'General site discussion'},
        list_=True,
    )
...
def test_view_categories_include_dead(app, authed_client):
    ...
    response = authed_client.get(
        '/forums/categories', query_string={'include_dead': True}
    )
    assert response.status_code == 200
...
def test_edit_category(app, authed_client):
    ...
    response = authed_client.put(
        '/forums/categories/1',
        data=json.dumps(
            {'name': 'Bite', 'description': 'Very New Description'}
        ),
    )
    check_json_response(
        response,
...
def test_edit_category_skips(app, authed_client):
    add_permissions(app, 'forums_view', 'forums_forums_modify')
    response = authed_client.put(
        '/forums/categories/1', data=json.dumps({'position': 0})
    )
    check_json_response(
        response,
        {
...
def test_delete_category(app, authed_client):
    ...
    check_json_response(
        response, 'ForumCategory 5 (uWhatMate) has been deleted.'
    )
...
@pytest.mark.parametrize(
    'endpoint, method',
    [('/forums/categories', 'GET'), ('/forums/categories', 'POST')],
)
