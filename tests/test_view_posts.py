...
def test_view_post(app, authed_client):
    ...
    check_json_response(response, {'id': 1, 'contents': '!site New yeah'})
...
def test_add_post(app, authed_client, monkeypatch):
    ...
    response = authed_client.post(
        '/forums/posts',
        data=json.dumps({'thread_id': 1, 'contents': 'hahe new forum post'}),
    )
    check_json_response(response, {'id': 9, 'contents': 'hahe new forum post'})
...
def test_add_post_locked(app, authed_client):
    ...
    response = authed_client.post(
        '/forums/posts',
        data=json.dumps({'thread_id': 3, 'contents': 'hahe new forum post'}),
    )
...
def test_add_post_length_limit(app, authed_client, monkeypatch):
    with mock.patch('forums.routes.posts.len', lambda _: 200000):
        ...
        response = authed_client.post(
            '/forums/posts',
            data=json.dumps(
                {'thread_id': 5, 'contents': 'hahe new forum post'}
            ),
        )
        check_json_response(
            response,
...
def test_add_post_double_post(app, authed_client):
    ...
    response = authed_client.post(
        '/forums/posts',
        data=json.dumps({'thread_id': 5, 'contents': 'hahe new forum post'}),
    )
    check_json_response(
        response,
        {
...
def test_add_post_nonexistent_thread(app, authed_client):
    ...
    response = authed_client.post(
        '/forums/posts',
        data=json.dumps({'thread_id': 100, 'contents': 'New Post'}),
    )
...
def test_edit_post_self(app, authed_client):
    ...
    response = authed_client.put(
        '/forums/posts/8', data=json.dumps({'contents': 'New site, yeah!'})
    )
    check_json_response(response, {'id': 8, 'contents': 'New site, yeah!'})
    ...
    assert (
        datetime.utcnow().replace(tzinfo=pytz.utc) - post_edits[0].time
    ).total_seconds() < 60
...
def test_edit_post_other_and_history(app, authed_client):
    add_permissions(
        app, 'forums_view', 'forums_posts_create', 'forums_posts_modify'
    )
    response = authed_client.put(
        '/forums/posts/7', data=json.dumps({'contents': 'snipped'})
    )
    check_json_response(response, {'id': 7, 'contents': 'snipped'})
...
def test_edit_post_other_no_contents_mod_advanced_locked_override(
    app, authed_client
):
    add_permissions(
        app, 'forums_view', 'forums_posts_create', 'forums_posts_modify'
    ...
    response = authed_client.put(
        '/forums/posts/2', data=json.dumps({'sticky': True})
    )
    check_json_response(
        response,
        {
...
def test_edit_post_locked_thread(app, authed_client):
    ...
    check_json_response(
        response, 'You cannot modify posts in a locked thread.'
    )
...
def test_edit_post_deleted_thread(app, authed_client):
    ...
    response = authed_client.put(
        '/forums/posts/1', data=json.dumps({'sticky': True})
    )
...
def test_edit_post_nonexistent(app, authed_client):
    ...
    response = authed_client.put(
        '/forums/posts/100', data=json.dumps({'sticky': True})
    )
